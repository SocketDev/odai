/**
 * @file CLI entry for the bench evaluator. Usage: node src/bench/run.mts # run
 *   the simulator backend through the odai interface node src/bench/run.mts
 *   --mock # run with single-response deterministic mock node src/bench/run.mts
 *   --backend=chrome-builtin # score a real registry backend through the same
 *   interface. The simulator mode lets the evaluator run in Node or node-smol
 *   without Chrome.
 */
